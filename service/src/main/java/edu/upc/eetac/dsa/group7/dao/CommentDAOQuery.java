...
public interface CommentDAOQuery {
    ...
    public final static String GET_COMMENTS = "select hex(id) as id, hex(creator) as creator, hex(restaurant) as restaurant, title, comment, likes, response, creation_timestamp from comments";
    ...
    public final static String REMOVE_VALORATION = "delete from like_restaurant where restaurant=unhex(?), user=unhex(?)";
}
